...
import { parseTrades as baseParseTrades } from '../base/parsers';
...
import { NULL_ID, ORDER_TYPE, MARKET_STATUS, ORDER_STATUS } from './constants';

export const parseTicker = (ticker, market = undefined, marketsById) => {
  let symbol;
...
export const parseTickers = (rawTickers, symbols = undefined) => {
  const tickers = [];

  rawTickers.forEach((rawTicker) => {
    tickers.push(parseTicker(rawTicker));
  });
...
export const parseOrder = (order, market, marketsById) => {
  let symbol;
  let timestamp;
  let scopedMarket = market;
...
export const parseOrders = (orders, market, since, limit) => {
  const result = Object.values(orders).map(order => parseOrder(order, market));

  return this.filterBySinceLimit(result, since, limit);
};

export const commonCurrencyCode = (currency) => {
  if (currency === 'BCC') return 'BCH';
  return currency;
};

export const currencyId = (currency) => {
...
export const parseMarkets = (markets, fees) => {
  ...
  markets.forEach((market) => {
    ...
    const base = commonCurrencyCode(baseId);
    const quote = commonCurrencyCode(quoteId);
...
export const parseTrade = (trade, market) => {
  ...
  if ('commission' in trade) {
    fee = {
      ...
      currency: commonCurrencyCode(trade.commissionAsset),
    };
  }
...
export const parseTrades = (trades, market, since, limit) =>
  baseParseTrades(trades, market, since, limit, parseTrade);
